...
import { canMoveToken, clampNumber, fail, isGm, ok, safeString, type Ctx } from './context.js';
...
export function updateToken(
  ...
  if (gm) {
    ...
    if (p.assetId !== undefined) token.assetId = p.assetId;
...
export function moveToken(ctx: Ctx, payload: TokenMovePayload, ack?: Ack<null>): void {
  ...
  token.x = Math.min(Math.max(x, -w), scene.width);
  token.y = Math.min(Math.max(y, -h), scene.height);
...
function applyExploration(ctx: Ctx, scene: Scene): void {
  ...
  // Apenas o mestre recebe o conjunto cru; o jogador recebe o resultado ja
  // calculado da propria visao em `refreshVision`.
  for (const socket of socketsOf(ctx.room)) {
    if (socket.data.viewer.role === 'GM') {
      socket.emit('fog:patch', { sceneId: scene.id, explored: { add: fresh } });
    }
  }
}
...
export function paintFog(ctx: Ctx, payload: FogPaintPayload, ack?: Ack<null>): void {
  ...
  let patch: Parameters<typeof emitFogToGm>[2] = {};
  ...
  switch (payload.action) {
    ...
    case 'hide': {
      ...
      // Esconder tambem apaga o que ja estava explorado, senao a area
      // continuaria em penumbra e o mestre nao teria como fecha-la de novo.
  ...
  ctx.room.touchScene(scene.id);
  emitFogToGm(ctx, scene, patch);
  refreshVision(ctx.room, scene);
  ok(ack, null);
}

function emitFogToGm(
  ctx: Ctx,
  scene: Scene,
  patch: { explored?: { add?: string[]; remove?: string[] }; revealed?: { add?: string[]; remove?: string[] }; hidden?: { add?: string[]; remove?: string[] } },
): void {
  for (const socket of socketsOf(ctx.room)) {
    if (socket.data.viewer.role === 'GM') socket.emit('fog:patch', { sceneId: scene.id, ...patch });
  }
}
...
export function updateTile(
  ...
  if (p.assetId !== undefined) tile.assetId = p.assetId;
...
export function updateEffect(
  ...
  if (p.assetId !== undefined) effect.assetId = p.assetId;
...
export function createWallHandler(ctx: Ctx, payload: { sceneId: string; wall: Partial<Wall> }, ack: Ack<Wall>): void {
  ...
  const wall = createWall({
    x1: clampNumber(w.x1, -30000, 30000, 0),
    y1: clampNumber(w.y1, -30000, 30000, 0),
    x2: clampNumber(w.x2, -30000, 30000, 0),
    y2: clampNumber(w.y2, -30000, 30000, 0),
    door: Boolean(w.door),
    open: Boolean(w.open),
...
export function updateWall(
  ...
  if (!gm) {
    ...
    if (p.door !== undefined) wall.door = Boolean(p.door);
    if (p.open !== undefined) wall.open = Boolean(p.open);
